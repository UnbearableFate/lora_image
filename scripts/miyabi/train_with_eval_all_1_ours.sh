...
CSV_PATH_DIR="experiments_all_ours"
...
for dataset in "${DATASETS[@]}"; do
  for n in "${nn[@]}"; do
    for seed in "${SEEDS[@]}"; do
      ...
      echo "==> Training ${dataset} with n=${n}, seed=${seed}, timestamp=${timestamp}"

      dataset_safe="${dataset//\//_}"
      train_log="${TRAIN_LOG_DIR}/train_${timestamp}_${dataset_safe}_s${seed}_n${n}.log"
